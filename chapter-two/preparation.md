...
* 明确项目的核心功能、亮点实现
* 阅读项目提供的 example、test 等模块，看是否包含核心功能、亮点实现
...
在阅读源码之前建议 `fork` `Nacos` 项目然后克隆下到你本地：
...
> 笔者使用的是 `SpringBoot 2.1.X` 以及 `SpringCloud Greenwich` 版本
...
执行完成之后在 `nacos_cofnig` 数据库下存在如下表：
...
修改 `nacos/console/src/main/resources/application.properties` 文件的中第 `29` 行开始 的数据库配置，笔者的配置如下：
